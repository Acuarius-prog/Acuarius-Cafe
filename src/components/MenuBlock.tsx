...
export default async function MenuBlock() {
  ...
  if (!categories || !items || items.length === 0) {
    return (
      <p className="carta-note">
        Aún no hay productos cargados. Agrégalos desde tu panel <code>/admin</code>.
      </p>
    );
  }

  return <MenuSection categories={categories} items={items} supabaseUrl={url} supabaseKey={key} />;
}
